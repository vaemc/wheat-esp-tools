<template>
  <div class="audio-page">
    <aside class="audio-sidebar panel">
      <header class="panel-head">
        <span class="panel-title">{{ $t("audio.toolbox") }}</span>
      </header>

      <div class="tool-list">
        <button
          v-for="tool in AUDIO_CONVERTER_TOOLS"
          :key="tool.id"
          type="button"
          class="tool-item"
      ...
      <div v-if="summary.count > 0" class="file-card">
        <div class="file-card-title">{{ $t("audio.batchSummary") }}</div>
        <div class="file-card-name">
          {{ $t("audio.batchCount", { n: summary.count }) }}
        </div>
        <div v-if="summary.done > 0" class="file-card-meta">
          {{ $t("audio.batchDone", { n: summary.done }) }}
        </div>
      </div>
    </aside>
...
import {
  AUDIO_CONVERTER_TOOLS,
  getAudioConverterTool,
...
.tool-icon[data-icon="wav2ogg"] {
  --icon-tint: rgba(54, 207, 201, 0.14);
}
