//! GIF → EAF 转换（对齐 esp_emote_gfx / esp_lv_eaf_player 格式）
...
pub type ProgressCallback = Box<dyn Fn(EafProgressEvent) + Send>;
...
fn quantize_color8(img: &RgbaImage) -> (Vec<u8>, Vec<u8>) {
    let mut rgba: Vec<u8> = Vec::with_capacity(img.as_raw().len());
    for p in img.pixels() {
...
fn encode_indexed_frame(
    ...
    let width = img.width();
    let height = img.height();
    let (indices, palette) = if bit_depth == 4 {
        quantize_gray4(img)
    } else {
        quantize_color8(img)
    };
...
pub fn convert_gif_to_eaf(
    ...
    let mut encoding_mode = options
        .encoding_mode
        .unwrap_or_else(|| "rle".into())
        .to_lowercase();
    let mut color_depth = options.color_depth.unwrap_or(8);
    let split_height = options.split_height.unwrap_or(32).max(1);
    ...
    emit_progress(
        ...
        1,
        5.0,
        &format!("已加载 {total_frames} 帧，开始编码…"),
    );
    ...
    for (index, frame) in frames.into_iter().enumerate() {
        ...
        // 编码阶段占 5% ~ 92%
        let percent = 5.0 + (current as f32 / total_frames.max(1) as f32) * 87.0;
        emit_progress(
            &on_progress,
            job_id,
            path,
        ...
        let key = resized.as_raw().clone();
        if let Some(existing) = cache.get(&key) {
            payloads.push(existing.clone());
            continue;
        }

        let payload = if encoding_mode == "jpeg" || color_depth == 24 {
            encode_jpeg_frame(&resized, split_height, jpeg_quality)?
        } else {
            encode_indexed_frame(&resized, split_height, color_depth, &encoding_mode)?
        };
